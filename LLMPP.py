...
class PluginManager:
    ...
    def manager(self, action: str, name: str = "") -> Any:
        """Unified plugin management entry point.

        Actions:
            list                -> list all discovered plugins with state
            enable <name>       -> enable a plugin (persist + load now)
            disable <name>      -> disable a plugin (persist + unload now)
            reload <name>       -> unload and reload a plugin

        This is the single entry point exposed to an authorized manager
        plugin (see config `manager_plugin`).
        """
        if action == "list":
            discovered = set()
...
def main():
    ...
    manager.manager_plugin = cfg.get("manager_plugin", "")
    manager.load()
